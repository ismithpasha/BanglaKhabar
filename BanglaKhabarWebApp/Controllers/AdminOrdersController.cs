...
namespace BanglaKhabarWebApp.Controllers
{
    ...
    public class AdminOrdersController : ApiController
    {
        ...
        public ResponseMessage GetOrderListByDate(ParaOrderByDate obj)
        {
            string reply = string.Empty;
            ResponseMessage rM = new ResponseMessage();
            List<AdminOrderInfo> orderList = new List<AdminOrderInfo>();
            ...
            try
            {
                ...
                var dt = adminRepository.GetOrderByDeleveryDate(obj.UserId.Trim(), obj.DeleveryDate.Trim(), ref reply);
                if (dt != null && dt.Rows.Count > 0) 
                {
                    foreach (DataRow dr in dt.Rows)
                    {
        ...
        public ResponseMessage GetOrderByCustomer(ParaOrderByDate obj)
        {
            string reply = string.Empty;
            ResponseMessage rM = new ResponseMessage();
            List<AdminOrderInfo> orderList = new List<AdminOrderInfo>();
            ...
            try
            {
                ...
                var dt = adminRepository.GetOrderByCustomer(obj.UserId.Trim(), obj.CustomerId.Trim(), ref reply);
                if (dt != null && dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
